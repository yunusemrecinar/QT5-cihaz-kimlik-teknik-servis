    ...
    if(database.isOpen()) {
        ...
        if(checkSeriNo) {
            if(qry.exec()) {
                ...
                setLog("[NOTE] mobiotdialog.cpp :"+ cihazSeriNo + "numaraları Cihaz Eklendi");
                qry.clear();
                qry.prepare("INSERT INTO cihazisim VALUES('"+ cihazSeriNo + "','" + musteriAdi + "');");
                if(qry.exec()) {
                    setLog("[NOTE] mobiotdialog.cpp :" + cihazSeriNo + "numaralı Yeni cihaz cihazisim tablosuna eklendi!");
                }else {
                    setLog("[ERROR] mobiotdialog.cpp : " + qry.lastError().text());
                }
                this->close();
