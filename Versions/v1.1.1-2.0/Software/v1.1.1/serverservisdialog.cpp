...
void ServerServisDialog::on_pushButton_clicked()
{
    if(database.open()) {
        ...
        if(qry.exec()) {
            ...
            setLog("[NOTE] serverservisdialog.cpp : " + servisNo + "numaralı server cihazı eklendi");
        }else {
        ...
        if(musteriCheck) {
            ...
            if(qry.exec()){
                ...
                setLog("[NOTE] serverservisdialog.cpp : "+ servisNo + "cihazının müşteri adı güncellendi");
                qry.clear();
