    ...
    if(database.isOpen()) {
        ...
        if(checkSeriNo) {
            if(qry.exec()) {
                ...
                setLog("[NOTE] serverdialog.cpp : " + cihazSeriNo +"numaralı Server eklendi");
                qry.clear();
                qry.prepare("INSERT INTO cihazisim(`Cihaz Seri No`,`İsim`) VALUES('"+ cihazSeriNo + "','" + musteriAdi + "');");
                if(qry.exec()) {
                    setLog("[NOTE] serverdialog.cpp :" + cihazSeriNo + "numaralı Yeni cihaz cihazisim tablosuna eklendi!");
                }else {
                    setLog("[ERROR] serverdialog.cpp : " + qry.lastError().text());
                }
                this->close();
